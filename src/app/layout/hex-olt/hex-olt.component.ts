...
@Component({
  selector: 'app-hex-olt',
  templateUrl: './hex-olt.component.html',
  styleUrls: ['./hex-olt.component.css']
})
export class HexOltComponent implements OnInit {
  tenantId = "";
  tabName : string = "HEX_OLT";
  circleName : any = "";
  tviSiteId : any = "";
  tviSiteIdList = [];
  ...
  getNoOfList(){
    this.allNoOfList = JSON.parse(localStorage.getItem("allNoOfList"));
      ...
      for(let i=0;i<this.allNoOfList.length;i++){
        ...
        else if(paramCode == "hexOlt_powerRequired"){
          let splitData = paramDesc.split(",");
          for(let i=0;i<splitData.length;i++){
            let json = {
              id : splitData[i]
  ...
  refreshPage(){
    this.route.navigateByUrl('/layout/dashbord', { skipLocationChange: true }).then(() => {
      this.route.navigate(['/layout/hex-olt']);
    }); 
  }
  ...
  validateActiveInfoDeta() : any{
    if(this.floorLength == ''){
      alert("please enter `Floor Space(in Sq . Meter)`");
      return false;
    }
    else if(this.powerRequired == ''){
      alert("please select `Power Required(In Watt)`");
      return false;
    }
    else if(this.fiberEntry == ''){
      alert("please select `Fiber Entry`");
      return false;
    }
    else if(this.noOfMCB == ''){
      alert("please select `MCB number`");
      return false;
    }
    else if(this.mcbRequiredInAmp == ''){
      alert("please select `MCB Required in Amp`");
      return false;
    }
